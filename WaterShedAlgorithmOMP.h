...
public:
	...
	//vector<WatershedPixel*> vec = watershedStructure.watershedStructure;
        std::queue<WatershedPixel*> pque;
        int curlab = 0;

        for (int h = HMIN; h < HMAX; ++h) {

	    #pragma omp parallel for num_threads(4) shared(pque) firstprivate(h, wsSize)  
		...
		int neighSize = neighbours.size();
                //parcurg vecinii
                #pragma omp parallel for num_threads(2) shared(pque) firstprivate(curdist, neighSize, p) 
	    ...
	    #pragma omp parallel for num_threads(4) shared(curlab) firstprivate(h, wsSize, pque)
            for (int pixelIndex = 0; pixelIndex < wsSize; pixelIndex++) {
                WatershedPixel* p = watershedStructure.at(pixelIndex);
